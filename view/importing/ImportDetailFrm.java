...
public class ImportDetailFrm extends JFrame implements ActionListener {
	...
	public ImportDetailFrm(Staff user, ImportInvoice importInvoice, Material selectedMaterial) {
	...
	private void refreshTable() {
		...
		ArrayList<ImportDetail> details = importInvoice.getDetails();
		for (int i = 0; i < details.size(); i++) {
			ImportDetail d = details.get(i);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(btnAdd)) {
			...
			try {
				...
				ImportDetail detail = new ImportDetail(selectedMaterial, qty, price);
			...
			try {
				if (dao.addImportInvoice(importInvoice)) {
					...
					JOptionPane.showMessageDialog(this,
							"Failed to save invoice! Check console for errors.",
